...
/*   Updated: 2020/05/18 15:49:17 by lrobino          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
t_cast perform_raycast(t_engine *eng, t_vec2d pos, t_vec2d dir, t_map map)
{
  ...
  {
    if (map.map[(int)grid_pos.x][(int)grid_pos.y]->id != CUB_AIR)
    ...
    {
      if (map.map[(int)(real.x) - ((cast.x == 0) ? 1 : 0)][(int)(real.y - ((cast.y == 0) ? 1 : 0))]->id != CUB_AIR)
