...
/*   Updated: 2020/05/14 22:21:48 by lrobino          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void        draw_ray_to_buffer(t_engine *eng, int x, int h, t_image tex, float offset, t_vec2d p_dir, float scale_f)
{
    ...
    while (y < eng->buf.size.y - 1)
    {
        ...
        col.channel.r = ft_constrain(ft_map(dist, MAX_VIEW, 0, 0, col.channel.r), 0, 255);
        col.channel.g = ft_constrain(ft_map(dist, MAX_VIEW, 0, 0, col.channel.g), 0, 255);
        col.channel.b = ft_constrain(ft_map(dist, MAX_VIEW, 0, 0, col.channel.b), 0, 255);
        if (pos < eng->buf.size.x * eng->buf.size.y)
        { 
            eng->buf.data[x + (y * (int)eng->buf.size.x)] = col.value;
        }
